...
feedback_cam::feedback_cam(processing_thread *thread, QWidget *parent)
  ...
  connect(proc_thread, &processing_thread::finished_analysis, this,
          &feedback_cam::finished_analysis);

  m_cam = &proc_thread->fb_cam;
...
void feedback_cam::setup_progress_box(int window) {
  ...
  QProgressDialog *progressbox =
      new QProgressDialog("Recording...", "Abort", 0, window, this);
  progressbox->setWindowTitle(QString("Recording..."));
  progressbox->setAttribute(Qt::WA_DeleteOnClose);
  connect(proc_thread, &processing_thread::update_progress, progressbox,
          &QProgressDialog::setValue);
  connect(proc_thread, &processing_thread::finished_analysis, progressbox,
