...
struct harmonics_filter {
  ...
  harmonics_filter(double freq, int filter_halfwidth) {
    ...
    std::vector<double> my_freqs = {freq,
                                    alias_compensated_freq(freq + 2 * freq)};
...
void processing_thread::test_loop_times() {
  ...
  int max_micros = 1000;
  int count = count_if(loop_times.begin(), loop_times.end(),
                       [max_micros](int n) { return n > max_micros; });

  emit write_to_log(
      QString("Mean value of loop times: " +
              QString::number(
                  std::accumulate(loop_times.begin(), loop_times.end(), 0) /
                  (double)loop_times.size())) +
      QString(" microseconds"));
  emit write_to_log(QString("Failure rate: ") +
                    QString::number(count / (double)loop_times.size() * 100) +
                    QString(" %"));
...
void processing_thread::test_loop_times_dual_cam() {
  ...
  emit write_to_log(QString("Feedback cam sensor readout: ") +
                    QString::number(fb_cam.SensorReadoutTime()) +
                    QString(" microseconds"));
  emit write_to_log(QString("Monitor cam sensor readout: ") +
                    QString::number(monitor_cam.SensorReadoutTime()) +
                    QString(" microsecpmds"));

  if (fb_cam.TriggerMode.GetValue() != TriggerMode_On |
      monitor_cam.TriggerMode.GetValue() != TriggerMode_On) {
    emit write_to_log(
        QString("Camera must be hooked up to trigger source in order to time"));
    emit finished_analysis();
    return;
...
void processing_thread::stream() {
  ...
  if (monitor_cam_enabled) {
    ...
    while (acquiring) {
      ...
      if (fb_cam.RetrieveResult(5, fb_ptr, Pylon::TimeoutHandling_Return))
        emit send_feedback_ptr(fb_ptr);

      if (monitor_cam.RetrieveResult(5, m_ptr, Pylon::TimeoutHandling_Return))
...
void processing_thread::analyze_spectrum() {
  ...
  std::ofstream reference("/home/loasis/Desktop/reference.txt",
                          std::ofstream::out);
...
void processing_thread::learn_total_system_response() {
  ...
  if (fb_cam.ResultingFrameRate.GetValue() < 1000) {
    ...
    generate_total_system_response_input(system_response_input.data(),
                                         max_DAC_val[1], 20, 300);
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int j = 1; j < tot_sys_response_window / 2; ++j) {
        ...
        phase[j] = atan2(driven_fft[i][tot_sys_response_window - j],
                         driven_fft[i][j]) -
      ...
      for (QVector<double> *vec : mag_phase) {
        ...
        gsl_filter_median(GSL_FILTER_END_PADVALUE, filtered, filtered,
                          med_filt);
  ...
  if (fb_cam.ResultingFrameRate.GetValue() < 1000) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int freq_idx = 0; freq_idx < 3; ++freq_idx) {
        ...
        double sum = std::accumulate(centroid_section.begin(),
      ...
      model_RMSE[i] = sqrt(arma::sum(square(errors)) / ddDAC_full.n_rows);

      vec simulated = centroid_full + errors;
...
void processing_thread::correlate_cameras() {
  ...
  emit write_to_log(
      "feedback_x = " + QString::number(cam_correlation_params[0][0]) +
      " monitor_x + " + QString::number(cam_correlation_params[0][1]));

  emit write_to_log(
      "feedback_y = " + QString::number(cam_correlation_params[1][0]) +
